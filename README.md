...
设置页的“积分”模块提供“赚裹酱”开关。开启后，模块会识别菜鸟客户端正常触发的裹酱初始化、任务完成校验、任务完成上报、喂养及奖励领取等流程节点，并将阶段和 API 名称写入全部运行日志。该功能不保存会话令牌，也不会重放网络请求。
...
该仓库设置为 **Public**，公开提供菜鸟智脑源代码、开发记录、自动构建配置和版本发布资料。
